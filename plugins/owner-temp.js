    ...
    if (now >= i.exitTime) {
      ...
      while (attempts < 3) {
        try {
          ...
          console.log(`[FRESITA TEMP] Sali del grupo: ${i.name}`)
          break
        } catch (e) {
          attempts++
...
let handler = async (m, { conn, args, command }) => {
  ...
  // COMANDO: TEMPLIST
  if (command === 'templist') {
    if (global.tempGroups.length === 0) return m.reply('🍓 *FRESITA BOT*\n❌ No hay grupos con temporizador activo.')
